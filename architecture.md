...
```ts
// The Graph から取得する生データ
export interface BottleGraphData {
  id: string
  tokenURI?: string
  sender?: string        // creator アドレス
}

// ユーザーが所有する小瓶（mybottles 用）
...
### 入力項目
- Filebase IPFS RPC API Key（単一フィールド）
...
### 表示要素
- Filebase IPFS RPC API Key 入力欄（password type）
- 保存ボタン
- 削除ボタン
...
│   ├── types/
│   ├── constants/
│   └── utils/          # subgraph / filebase / encryption / wagmi など副作用ありを含む
```

## 2. 責務ルール

- `components/`: UI 表示のみ。可能な限り副作用を持たせない
- `hooks/`: UI とロジックの橋渡し。フォーム制御や非同期処理の統合
- `stores/`: Zustand による状態保持。設定値や UI 状態を持つ
- `utils/`: コントラクト呼び出し、The Graph、Filebase、暗号化 I/O、純粋関数を含む
...
### 必要クエリ 1: 海にある小瓶一覧
- エンティティ: `driftingBottles`
- フロント側で `sender != currentUser` を除外
- 必須フィールド: id, tokenURI, sender

### 必要クエリ 2: 自分が拾った小瓶一覧
- エンティティ: `bottleClaimeds(where: { claimer: currentUser })`
- ソート: `blockTimestamp desc`
- tokenURI 解決に `bottleMinteds` を結合
